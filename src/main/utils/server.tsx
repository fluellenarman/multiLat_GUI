...
ipcMain.on('missileHit', () => {
  sendMissileHitRedGUI()
})
...
async function sendMissileHitRedGUI() {
    const redPort = 3000
    const localhost_url = `http://localhost:${redPort}/missileHit`
    try {
        let targetURL = localhost_url
        console.log(networkURL)
    if (networkURL != '') {
        targetURL = `${networkURL}/missileHit`
        console.log('Using network URL: ', networkURL)
    }
        console.log(`Sending missile hit to ${targetURL}`)
        await fetch(targetURL)
    } catch (error) {
        console.error('Error in launcherLocQuery():', error)
    }
