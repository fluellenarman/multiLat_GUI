...
const TestDroneButton: Component = () => {
	...
	function handleChange(event: Event) {
		...
		switch (target.value) {
			case 'path0':
				testDrone.currentPath.push({ x: 10, y: 10, z: 10 })
				break
			case 'path1':
				testDrone.currentPath.push({ x: 50, y: 50, z: 10 }, { x: 500, y: 50, z: 10 })
				break
			case 'path2':
			...
			case 'path3':
				...
				testDrone.currentPath.push({ x: 10, y: 10, z: 10 }, { x: 10, y: 10, z: 100 })
				break
			case 'path4':
		...
		<div>
			<Show
				...
				fallback={
					<button
						onClick={handleClick}
						class="icon-button drone-button"
						data-tooltip="Reset Drone"
					>
						<svg
							fill="none"
							stroke="currentColor"
						...
						>
							<path d="M10 10h4v4h-4v-4M10 10 6.5 6.5M9.96 6A3.5 3.5 0 1 0 6 9.96M14 10l3.5-3.5M18 9.96A3.5 3.5 0 1 0 14.04 6M14 14l3.5 3.5M14.04 18A3.5 3.5 0 1 0 18 14.04M10 14l-3.5 3.5M6 14.04A3.5 3.5 0 1 0 9.96 18"></path>
						</svg>
					</button>
				}
			>
				<button
					onClick={handleClickTestMode}
					class="icon-button drone-button"
					data-tooltip="Reset Drone"
				>
					<svg
			...
			<Show when={TestingMode() == true}>
				<select onChange={(event) => handleChange(event)} class="select-field drone-field">
					<option value="path0">Select a path...</option>
