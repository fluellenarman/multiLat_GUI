class MissileState {
	...
	findNextPoint() {
		...
		if (this.lifeCycle == 2 && this.LOSonMidcourse == false) {
			console.log(
				'Missile in terminal phase, no LOS on target. Continuing in current direction.'
			)
...
class DroneTracker {
	...
	calculateQuadrant(ctx: CanvasRenderingContext2D, x: number, y: number) {
		...
		if (currentQuadrant !== this.quadrant) {
			console.log(
				...
				currentQuadrant,
				'Letter: ',
				this.quadrantLetter.get(currentQuadrant)
			)
	...
	testBehavior() {
		setInterval(() => {
			...
			if (this.shouldDrawLine == true) {
				this.LOS_achieved = true
...
function drawLauncherDirection(ctx: CanvasRenderingContext2D) {
	...
	const end = pointFromAngleAndDistance(
		missile.launcherX,
		missile.launcherY,
		missile.initialDirection,
		40
	)
...
function pointFromAngleAndDistance(
	x: number,
	y: number,
	angle: number,
	distance: number
...
function pointFromAngleDistanceTurningRate(
	x: number,
	y: number,
	angle: number,
	distance: number,
...
function d3_distanceBetweenPoints(
	x1: number,
	y1: number,
	z1: number,
	x2: number,
...
function zPointFromDestination(
	currentZ: number,
	destZ: number,
	climbSpeed: number,
	fallSpeed: number
